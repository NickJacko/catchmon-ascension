...
/** Retries arrival with different seeds until a customer with a request is generated (bounded, deterministic per seed). */
...
describe("STANDARD_SALE", () => {
  it("credits Coins, consumes stock, gains Momentum, awards rank progress, and resolves the customer", () => {
    ...
    expect(result.value.nextState.economy.coins).toBe(
      state.economy.coins + options.standardSale.finalCoins,
    );
    expect(getTotalQuantity(result.value.nextState.inventory, itemId)).toBe(
      stockBefore - 1,
    );
    expect(result.value.nextState.shop.momentum).toBe(
      state.shop.momentum + options.standardSale.momentumDelta,
    );
    expect(result.value.nextState.progression.rankProgress).toBeGreaterThan(
      state.progression.rankProgress,
    );
    ...
    expect(result.value.events).toEqual([
      expect.objectContaining({ kind: "STANDARD_SALE_RESOLVED", customerId }),
    ]);
  });

  it("fails with a typed error and does not mutate state for an unknown customer", () => {
  ...
  it("fails with OUT_OF_STOCK and leaves the customer active when stock runs out before the sale", () => {
    const { state, customerId } = stateWithRequestingCustomer();
    const customer = state.customers.customers[customerId]!;
    const itemId = productItemId(
      customer.requestedProductId!,
    ...
    const totalStock = getTotalQuantity(state.inventory, itemId);

    // Drain all stock via repeated sales-shaped inventory removal is not
    // exposed directly; simulate "no stock" by zeroing the stack directly.
    ...
    expect(totalStock).toBeGreaterThan(0); // sanity: there was stock before draining
    ...
    expect(drained.customers.customers[customerId]).toBeDefined();
  });
});
